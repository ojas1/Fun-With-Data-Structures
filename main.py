    ...
    def replace_data(self, opt, btn):
        try:
            if opt == 'addr':
                ...
                if pos < len(ds):
                    ds[pos].val.text = self.replacement.text
            else:
                data = int(self.pos_or_data_to_replace.text)
                for i in range(len(ds)):
                    if int(ds[i].val.text) == data:
                        ds[i].val.text = self.replacement.text
        except:
            print "error"

class LLHeadWidget(BoxLayout):
    pass

class LLElementWidget(BoxLayout):
    pass
...
class LLDeletionWidget(BoxLayout):
    def delete_data(self, pos):
        ...
        try:
            ...
            return
        if pos <0 or pos > len(ds)-1:
            return 

        if len(ds) > 0:
            action_widget.disp_sec.remove_widget(ds[pos])
            if pos == 0:
                head_of_list.pointsto.text = ds[pos].nextptr.text
            else:
                ds[pos-1].nextptr.text = ds[pos].nextptr.text
            del ds[pos]
...
class LLReplacementWidget(BoxLayout):
    def replace_node(self, pos):
        ...
        if len(ds) > 0:
            ...
            if pos == 0:
                head_of_list.pointsto.text = ds[pos].node_address.text
            else:
                ds[pos-1].nextptr.text = new_node.node_address.text
            ds[pos] = new_node
            action_widget.disp_sec.clear_widgets()
            action_widget.disp_sec.add_widget(head_of_list)
            for i in range(len(ds)):
...
class QueueReplacementWidget(BoxLayout):
    pass
    
class StackElementWidget(BoxLayout):
    pass
...
class StackReplacementWidget(BoxLayout):
    pass

class HeapElementWidget(BoxLayout):
    pass
...
class HeapInsertionWidget(BoxLayout):
    pass

class HeapDeletionWidget(BoxLayout):
    pass

class HeapReplacementWidget(BoxLayout):
    pass


class TreeElementWidget(BoxLayout):
    pass
...
class TreeInsertionWidget(BoxLayout):
    pass

class TreeDeletionWidget(BoxLayout):
    pass

class TreeReplacementWidget(BoxLayout):
    pass


class GraphElementWidget(BoxLayout):
    pass
...
class GraphInsertionWidget(BoxLayout):
    pass

class GraphDeletionWidget(BoxLayout):
    pass

class GraphReplacementWidget(BoxLayout):
    pass
...
class DataStrWidget(BoxLayout):
    def __init__(self, **kwargs):
        super(DataStrWidget, self).__init__(**kwargs)   

# home page list button
class DSButton(ListItemButton):
    pass
